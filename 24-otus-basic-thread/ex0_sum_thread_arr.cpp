...
int main(int argc, char * argv[])
{
    ...
    std::vector<unsigned long long> sum(n_threads, 0);
    unsigned long long step = N / n_threads;

    std::vector<std::thread> thrs;
    for (int i = 0; i < n_threads; ++i) {
        thrs.emplace_back(make_summation, i * step, (i + 1) * step, std::ref(sum[i]));
    }
    ...
    unsigned long long reduce_sum = 0;
    ...
    std::cout << "Final result = " << reduce_sum << std::endl;
